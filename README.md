...
Connect a **TypeScript/JavaScript frontend to a Go backend** without hand-writing
`json:"..."` tags or maintaining duplicate DTOs. Keep idiomatic Go structs on the
server and idiomatic camelCase on the client — the conversion happens at the
network layer, in one line.

```
        camelCase JSON                         snake_case
   ┌──────────────────────┐   request    ┌──────────────────────┐
   │  TS / JS frontend     │ ───────────▶ │  jsonflex.Middleware  │ ──▶ your Go handler
   │  { "userName": ... }  │              │  camelCase→snake_case │     { "user_name": ... }
   │                       │ ◀─────────── │  snake_case→camelCase │ ◀── (plain snake_case
   └──────────────────────┘   response    └──────────────────────┘      structs & tags)
```
...
```go
// BEFORE — a json tag on every field, on every struct, kept in sync by hand:
type CreateUserRequest struct {
    ...
    // ...50 more fields, all tagged twice in your head
}

// AFTER — plain structs (or snake_case tags you already have), one middleware:
...
- **Fast** — the built-in directions use a byte-level engine: ~1 allocation per
  body (see [Benchmarks](#benchmarks)).
...
## When to use this (and when struct tags are enough)

To be upfront: for a **typed API where you own the structs**, plain `json`
tags — generated by a tool like
[gomodifytags](https://github.com/fatih/gomodifytags) — are a great, zero-runtime
solution, and often the right default. jsonflex earns its place where tags are
painful or impossible:

- Your Go code and DB use **snake_case** but the frontend expects **camelCase**,
  and you don't want to rewrite every tag to camelCase.
- You handle **dynamic / untyped JSON** (`map[string]any`, a proxy, a gateway,
  passthrough payloads) — there is no struct to tag.
- You want **one casing policy applied globally**, including nested and
  third-party types you can't easily tag.

If you own clean structs and care about the last microsecond, generate the tags.
If you're fighting untyped or unowned JSON, or want zero tags, use the middleware.
...
Every claim above is backed by tests (`go test -race ./...`). Highlights:

- **Correctness** — recursion over nested objects/arrays (to 50+ levels), large
  arrays, every JSON scalar type, unicode keys, and camelCase↔snake_case
  round-trips.
- **Value fidelity** — exact big-integer precision (`9223372036854775807`,
  `2⁵³+1`), no HTML escaping, and preserved key order.
- **Two engines cross-checked** — a differential test runs a corpus through both
  the byte-level and tree-walk engines and asserts they agree.
- **Safe passthrough** — content-type gating (`application/json`, `+json`,
  charset params) and empty/invalid JSON left unchanged, never erroring.
- **Efficiency, enforced** — an allocation test fails the build if the byte
  engine regresses above a small constant (measured: 1 alloc vs 154 for
  tree-walk on the same payload).
- **Concurrency** — the `Converter` is exercised from many goroutines under the
  race detector.
